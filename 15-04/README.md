...
1. Внутри контейнера я изменил порт прослушивания nginx с 80 на 81
2. Снаружи контейнера проброс портов настроен как 127.0.0.1:8080 → 80 (контейнер)
3. Контейнер пробрасывает хост-порт 8080 на контейнерный порт 80. Но внутри контейнера nginx теперь слушает на порту 81. Получается разрыв связи: запросы с хоста на 127.0.0.1:8080 перенаправляются на порт 80 контейнера, но там ничего не слушает.
4. Результат: curl http://127.0.0.1:8080 возвращает ошибку соединения.
